 ...
 *
 *   - string: navegar directo a esa ruta (`/facturas` o `/clientes`)
 *   - object: NO navegar, abrir el modal indicado (sin firmas → primera vez)
 */
export type PostLoginRoute = '/facturas' | '/clientes' | { modal: 'crear-empresa' };
 ...
 *   - solo firmas con `tipo_siigo='contador'`      → '/clientes'
 *
...
export function resolvePostLoginRoute(firmas: Firma[]): PostLoginRoute {
  ...
  return hasNube ? '/facturas' : '/clientes';
}